...
function form_submit_validation(myForm, submit_form)
{
	...
	if(submit_form){
		setTimeout(
		  function() 
		  {
		    if($('#' + myForm + ' .err-color').length > 0)
			{
				$('.err-color').each(function(){
					goToByScroll(this);
					return false;
				});	
			} else {
...
function in_line_validation()
{
	...
	$('.textarea').blur(function(){
		validate_text_area_field(this, true);		
	});
...
function check_add_parent(input_id,input_type,input_class)
{
    if($('#' + input_id).parent().hasClass('form-field') || $('#' + input_id).parent().hasClass('form-group') || $('#' + input_id).parent().hasClass('field-box')){
		   ...
		   var small_block = $('#' + input_id).parent().hasClass('small-error-block'); 
		   if(small_block)
		   {
				if(input_type == '1'){
						$('#' + input_id).parent().addClass(input_class);
...
{	
	...
	check_add_parent(id_input, 1, 'has-error');
        ...
        if(isInputGroup)
        {
            ...
            $('#' + id_input).parent().parent().append('<span class="help-block err-color pull-left"><i class="fa fa-times-circle"></i> ' + text_notify + '</span>');	
        }
        else
        {
            $('#' + id_input).next().remove();
            $('#' + id_input).parent().append('<span class="help-block err-color pull-left"><i class="fa fa-times-circle"></i> ' + text_notify + '</span>');	
        }
...
function validate_text_area_field(obj, is_auto_save)
{
	var require_value = $.trim($(obj).val());
	$(obj).val(require_value);
	var id = $(obj).attr('id');
		...
		if($.isNumeric(number) && /^[0-9]+$/.test(number))
		{
			if(number == parseInt(number))
			{
				...
				else
				{
                                    ...
                                    {
                                        display_error_notification(id, "This field should be greater than zero.");
                                    }
                                    else
                                    {
...
function validate_min_length_field(obj, is_auto_save)
{
	...
	{		
		...
		if(value.length < min)
		{
			notify = "This field must be at least " + min + " characters in length.";
			display_error_notification(id, notify);
		}
...
function validate_re_match_field(obj, is_auto_save)
{
	...
	{		
		...
		{
			notify = "This field does not match.";
			display_error_notification(id, notify);
		}
		else
		{
...
function validate_numeric_field(obj, is_auto_save)
{
	...
	{		
		notify = "This field must be numeric.";
		display_error_notification(id, notify);
	}
	else if(parseInt(require_value) == 0)
	{		
		notify = "This field must be gretaer than 0.";
		display_error_notification(id, notify);
	}
	else
	{
...
function validate_website_url(obj, is_auto_save)
{
    ...
    {		
            ...
            {	
                notify = "This field doesn't look like a valid web address.";
                display_error_notification(id, notify);
            }
            else
            {
...
$(".domain").keyup(function () {
    var url = $('.domain').val();
    var setDomainValue = $('.setDomain').val();
    isvalid = validurl(url);
    if (isvalid)
    {
        if (url.indexOf("//") > -1) {
        ...
        getWeb = hostname.split('.')
        indexNumber = getWeb[0] == "www" ? 1 : 0;
        personalizedWebAddress = getWeb[indexNumber];
        $('.setDomain').val(personalizedWebAddress);
    }
});
...
function showHidePassword(){
    if($("#password").attr('type') == "password"){
        ...
        $("#showHidePassword").html("");
        $("#showHidePassword").html("Hide Password");
    }else{
        $("#password").attr('type','password');
        $("#showHidePassword").html("");
        $("#showHidePassword").html("Show Password");
    }
}

function checkFormBtnState(){
    
    if($.trim($("#szFirstName").val()) != '' && $.trim($("#szLastName").val()) != '' && $.trim($("#password").val()) != ''){
        
        if(($("#letter").hasClass("valid") == true) && ($("#capital").hasClass("valid") == true) && ($("#number").hasClass("valid") == true) && ($("#length").hasClass("valid") == true)){
            $("#saveUserData").removeAttr('disabled');
        }else{
